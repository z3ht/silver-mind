...
class DeepChess(ChessNet):
    ...
    def _create_base_model(self, X, y):
        model = models.Sequential()
        for i in range(1, len(self.dbn_layers)):
            ...
            train_model.fit(X, y, batch_size=self.dbn_batch_size, epochs=self.dbn_epochs)
    ...
    def _create_model(self, X, y, base_refresh=False, base_model_path="models/dbn.tf"):
        base_model_path = __file__ + f"/../{base_model_path}"
        if base_refresh or not os.path.isfile(base_model_path):
            base = self._create_base_model(X, y)
    ...
    def train(self, X, y, base_refresh=False, base_model_path="models/dbn.tf", num_samples=800000):
        ...
        model = self._create_model(X=X, y=y, base_refresh=base_refresh, base_model_path=base_model_path)
        model.summary()

        history = model.fit(X, y, use_multiprocessing=True, epochs=self.epochs, batch_size=self.batch_size)
